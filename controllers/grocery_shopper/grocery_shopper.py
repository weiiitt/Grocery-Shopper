...
def calculateIk(target_position, initial_position, orient=False, orientation_mode="Y", target_orientation=None):
...
def arm_controller(key):
    ...
    #change axis for ik control
    # ik_axis = "Y"
    # if key_char == END_EFFECTOR_CONTROL_KEYS["orient_y"]: 
    #     ik_axis = "Y"
    #     print("Changed Axis to: ", ik_axis)
    # elif key_char == END_EFFECTOR_CONTROL_KEYS["orient_x"]: 
    #     ik_axis = "X"
    #     print("Changed Axis to: ", ik_axis)
    # elif key_char == END_EFFECTOR_CONTROL_KEYS["orient_z"]: 
    #     ik_axis = "Z"
    #     print("Changed Axis to: ", ik_axis)

    # 1. Get Current Joint Positions for the IK chain (with clamping and validation)
    num_links = len(my_chain.links)
    initial_position = np.zeros(num_links) # Initialize with zeros
    valid_initial_pos = True
    SENSOR_BOUND_TOLERANCE = 0.01 # Allow small deviation outside bounds before warning

    for i in range(num_links):
        if my_chain.active_links_mask[i]:
            link = my_chain.links[i] # Get the link object
            link_name = link.name
            if link_name in motor_dict:
                try:
                    sensor_value = motor_dict[link_name].getPositionSensor().getValue()

                    if not np.isfinite(sensor_value):
                        print(f"!!! ERROR: Non-finite sensor value ({sensor_value}) read for {link_name}. Skipping step.")
                        valid_initial_pos = False
                        break # Stop processing if any sensor is bad

                    # Get joint bounds from ikpy chain
                    lower_bound, upper_bound = link.bounds

                    # Check if sensor value is significantly outside bounds
                    ...
                    (upper_bound is not None and sensor_value > upper_bound + SENSOR_BOUND_TOLERANCE):
                        print(f"!!! WARNING: Sensor value {sensor_value:.4f} for {link_name} is significantly outside URDF bounds [{lower_bound:.4f}, {upper_bound:.4f}]. Clamping.")
                        # Optionally, you could choose to 'continue' or 'break' here if this is critical

                    # Clamp the sensor value to be strictly within bounds for IK initial guess
                    if lower_bound is not None and upper_bound is not None:
                        initial_position[i] = np.clip(sensor_value, lower_bound, upper_bound)
                    elif lower_bound is not None:
                        initial_position[i] = max(sensor_value, lower_bound) # Clamp only lower
                    elif upper_bound is not None:
                        initial_position[i] = min(sensor_value, upper_bound) # Clamp only upper
                    else:
                        # No bounds defined for this joint in URDF
                        initial_position[i] = sensor_value

                except Exception as e:
                    print(f"Error reading sensor or getting bounds for {link_name}: {e}. Skipping step.")
                    valid_initial_pos = False
                    break # Stop processing on error

    if not valid_initial_pos:
        return # Exit if initial positions couldn't be read reliably

    # 2. Forward Kinematics to find current end-effector pose
    try:
        current_fk = my_chain.forward_kinematics(initial_position) # Use clamped initial position for FK too
        if not np.all(np.isfinite(current_fk)):
            ...
            print(f"  Initial Position (clamped) causing issue: {initial_position}") # Use clamped value in log
            return
        ...
        print(f"  Initial Position (clamped) used: {initial_position}") # Use clamped value in log
        return
    ...
    # Pass the *clamped* initial_position to IK
    ik_results = calculateIk(
        target_position,
        initial_position=initial_position, # Use the clamped initial position
    ...
    if ik_results is not None:
        ...
        print(f"  Initial Guess (clamped) used: {initial_position}") # Log the clamped guess
